...
def create_user_session_db(
    ...
    try:
        session = UserSession(
            ...
            expires_at=expires_at,  # FIXED: Now included
            is_active=True,
            trust_score=None,
        ...
        logger.info(f"✅ Session created: {session_token} for user {user_id}")
        return session
...
async def create_user_session(
    ...
    """Create new user session - FIXED VERSION"""
    try:
        ...
        session = create_user_session_db(
            ...
            session_token=session_token,
            expires_at=expires_at  # FIXED: Now provided
        )
...
async def get_session_status(
    ...
    try:
        ...
        return {
            ...
            "status": "active" if session.expires_at > datetime.utcnow() else "expired"
        }
    ...
    try:
        ...
        for session in sessions:
            session_list.append({
                ...
                "last_activity": session.last_activity.isoformat(),
                "expires_at": session.expires_at.isoformat(),
                "is_active": session.expires_at > datetime.utcnow(),
                "trust_score": session.trust_score,
                "is_mirage_active": session.is_mirage_active
            })
